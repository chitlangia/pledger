...
#define FRACTION_RL_LEN (READ_LENGTH*3)/4


__kernel void pledger(__global uchar* genome, __global uchar* Reads_fwd, __global uchar* Reads_rev, __global uint* SA, __global uint* tally, __global uint* F,__global uint* cand_locs_per_read, __global uchar* genomic_strand_and_ED_for_mapped_reads, __global uint* endpos_for_mapped_reads, __global ushort* t_offset)
{	
	...
	uint tally_row, tally_row_val1, tally_row_val2;

	// Processing reverse complement of the reads
	...
	for(beginning = READ_LENGTH - (ERROR*MIN_QG_LEN) - 1; beginning > MIN_QG_LEN - 2; beginning--)
	{
		...
		{
			tally_row = ((sa_start_pos + occurences - 1)>>4);
			tally_row_val1 = tally[(tally_row*NO_OF_ALPHABETS) + R[current_prefix_pos]];
			tally_row_val1 = tally_row_val1 + ((t_offset[sa_start_pos + occurences - 1]>>(4*R[current_prefix_pos]))&15);


			tally_row = ((sa_start_pos - 1)>>4);
			tally_row_val2 = tally[(tally_row*NO_OF_ALPHABETS) + R[current_prefix_pos]];
			tally_row_val2 = tally_row_val2 + ((t_offset[sa_start_pos - 1]>>(4*R[current_prefix_pos]))&15);
			occurences = tally_row_val1 - tally_row_val2;
			sa_start_pos = local_F[R[current_prefix_pos]] + tally_row_val2;
			if(occurences == 0)
				{break;}
		}
	...
	for(i = 0; i <= ERROR; i++)
	{
		...
		occurences = (occurences > 500)?500:occurences;
	...
	if(no_of_locations >= CAND_LOC_PER_READ)
	{
...
__kernel void preprocess_read(__global char* Reads, __global uchar* Reads_fwd, __global uchar* Reads_rev)
{
	uint gid = get_global_id(0)*READ_LENGTH;
	int j;
	...
	{	
		switch(Reads[gid + j])
		{
			case 'T':
				Reads_fwd[gid + j] = 3;
				Reads_rev[gid + READ_LENGTH-1-j] = 0;
				break;
			case 'C':
				Reads_fwd[gid + j] = 1;
				Reads_rev[gid + READ_LENGTH-1-j] = 2;
				break;
			case 'G':
				Reads_fwd[gid + j] = 2;
				Reads_rev[gid + READ_LENGTH-1-j] = 1;
				break;
			case 'A':
				Reads_fwd[gid + j] = 0;
				Reads_rev[gid + READ_LENGTH-1-j] = 3;
				break;
			default: //case 'A': 'N' is also being treated as 'C'
				Reads_fwd[gid + j] = 1;
				Reads_rev[gid + READ_LENGTH-1-j] = 2;
				break;
		}
